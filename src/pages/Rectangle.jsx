...
const Rectangle = () => {
  ...
  const { msg } = alert;
  const { area, perimeter, rectangle } = useOptions();

  const handleSubmit = async (e) => {
    e.preventDefault();

    const result = await rectangle(base, height);
  ...
  return (
    <div className="lg:w-1/2 mx-auto">
      <h1 className="text-white text-center font-bold text-2xl uppercase">Rectángulo</h1>
      <div className="md:grid grid-cols-2 mt-5 bg-slate-800 p-5 rounded-lg">
        ...
        <div>
          ...
          (
            <p className="text-white text-center mt-9">El resultado aparecerá aquí</p>
          )}
        </div>
      </div>
